...
Participants' gameplay variables were submitted to parallel analysis. However, factor extraction resulted in Heywood cases. Each was analyzed singly as a potential predictor of aggressive behavior. Players with more kills/zorches gave lower coldpressor assignments (`r report_t(ex1, "Game.2", "b")`). Players firing more total combined bullets and shotgun shells also gave lower coldpressor assignments (`r report_t(ex2, "I(Game.4 + Game.5)", "b")`). No other effects were statistically significant. Because these are exploratory tests and would not survive multiple comparison correction, we do not offer further consideration of these effects.

```{r, echo = F, warning = F}
lm(DV ~ Violence*Difficulty*I((L2d4d_std + R2d4d_std)/2), data = dat) %>% 
  summary()
```

### Manipulation checks

Here's the full output for the effect of condition on the manipulation checks.

"The game involved a great deal of violence":
```{r, echo = F}
summary(lm(violence ~ Difficulty * Violence, data = dat))
```

"I felt I behaved aggressively during the game":
```{r, echo = F}
summary(lm(aggressed ~ Violence*Difficulty, data = dat))
```

Latent factor representing challenge (challenging, stressful, violence, reflexes, difficult, mental effort, exhausting):
```{r, echo = F}
...
Here's a table of Kendall's Tau.
...
### Summary stats
Here's a big table of all kinds of summary stats.
```{r, echo = F}
dat %>% 
