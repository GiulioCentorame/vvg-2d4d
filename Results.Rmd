...
## Manipulation Check
*Game manipulation.* Participant ratings on the post-questionnaires were submitted to 2 (Violence) × 2 (Difficulty) ANOVA. The manipulation was highly effective: participants indicated that the violent game (*M* = `r vioMeans["Violent"] %>% signif(2)` (`r vioSDs["Violent"] %>% signif(2)`)) was much more violent than the nonviolent game (*M* = `r vioMeans["Less Violent"] %>% signif(2)` (`r vioSDs["Less Violent"] %>% signif(2)`); `r reportCI(smd = (vioMeans["Violent"] - vioMeans["Less Violent"]) / pool.sd(vioSDs, vioN), n.1 = vioN[2], n.2 = vioN[1])`). Similarly, participants rated the difficult game as being more challenging than the easy game, *t*(243) = 6.15, *p* < .001, *d* = 0.74, [0.50, 0.99]. The difficult game was not seen as more violent, nor the violent game seen as more difficult.

Players generally did not perceive themselves to have behaved aggressively during the game (M = 3.6 (1.78) in the violent game, M = 2.76 (1.55) in the less-violent game, where 4 represents "Neither agree nor disagree"). Ratings of one's own aggressive behavior were significantly influenced by the game's violent content (`r report_t(check1.1, "Violence1", "d", 136, 139)`), but not by the game's difficulty (`r report_t(check1.1, "Difficulty1", "d", 138, 137)`). An interaction was observed such that the difference between violent and non-violent games was larger when the game was easy, `r report_t(check1.1, "Violence1:Difficulty1", "d", 250/2, 250/2)`.
...
If violent games increase hostile appraisals, one might expect participants in the violent-game condition to rate their interaction more negatively. A 2 (Violence) × 2 (Difficulty) ANOVA was conducted to determine whether the game played influenced participants’ ratings of the interaction. Effects were small and not statistically significant (violence, `r report_t(m.provoke, "Violence1", "d", 136, 139)`; difficulty, `r report_t(m.provoke, "Difficulty1", "d", 138, 137)`; Violence × Difficulty, `r report_t(m.provoke, "Violence1:Difficulty1", "d", 69+70, 67+69)`), suggesting that the game played had a minimal influence on participants’ reaction to the feedback. 
...
Higher-order interactions were not supported by the data, either. Evidence was ambiguous regarding a Violence × Difficulty interaction (BF = `r (bfList[4]/bfList[3]) %>% round(2)` favoring the null). Neither violence nor difficulty interacted with 2D:4D of the left hand (BF = `r bf_grab(bf01_vio_l2d4d)`, `r bf_grab(bf01_diff_l2d4d)`, respectively) or 2D:4D of the right hand (BF = `r bf_grab(bf01_vio_r2d4d)`, `r bf_grab(bf01_diff_r2d4d)`).  The Violence × Difficulty × 2D:4D interaction was not supported (left-hand BF = `r bf_grab(bf01_3way_l2d4d)`, right-hand BF = `r bf_grab(bf01_3way_r2d4d)`).

Experienced provocation was added to the model as a predictor. An effect of provocation was strongly supported by the evidence (BF = `r bf_grab(bf7.4) %>% round(2)`). However, addition of this covariate did not improve the strength of evidence for main effects of violence (BF = `r bf_grab(bf01_vio_cov)`), difficulty (BF = `r bf_grab(bf01_diff_cov)`), or 2D:4D (BF = `r bf_grab(bf01_l2d4d_cov)`, left hand; BF = `r bf_grab(bf01_r2d4d_cov)`, right hand). Taken together, these results indicate that aggression could be predicted by experienced provocation but not by game condition.
