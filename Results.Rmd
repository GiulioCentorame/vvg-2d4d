...
## Manipulation Check
*Game manipulation.* Participant ratings on the post-questionnaires were submitted to 2 (Violence) × 2 (Difficulty) ANOVA. The manipulation was highly effective: participants indicated that the violent game (*M* = `r vioMeans["Violent"] %>% signif(2)` (`r vioSDs["Violent"] %>% signif(2)`)) was much more violent than the nonviolent game (*M* = `r vioMeans["Less Violent"] %>% signif(2)` (`r vioSDs["Less Violent"] %>% signif(2)`); `r reportCI(smd = (vioMeans["Violent"] - vioMeans["Less Violent"]) / pool.sd(vioSDs, vioN), n.1 = vioN[2], n.2 = vioN[1])`). A latent factor was made to represent experienced challenge. Participants rated the difficult game as being more challenging than the easy game, *t*(243) = 6.15, *p* < .001, *d* = 0.74, [0.50, 0.99]. The difficult game was not seen as more violent, nor the violent game seen as more difficult.
...
Higher-order interactions were not supported by the data, either. Evidence was ambiguous regarding a Violence × Difficulty interaction (BF = `r (bfList[4]/bfList[3]) %>% round(2)` favoring the null). Neither violence nor difficulty interacted with 2D:4D of the left hand (BF~01~ = `r bf_grab(bf01_vio_l2d4d)`, `r bf_grab(bf01_diff_l2d4d)`, respectively) or 2D:4D of the right hand (BF~01~ = `r bf_grab(bf01_vio_r2d4d)`, `r bf_grab(bf01_diff_r2d4d)`).  The Violence × Difficulty × 2D:4D interaction was not supported (left-hand BF~01~ = `r bf_grab(bf01_3way_l2d4d)`, right-hand BF~01~ = `r bf_grab(bf01_3way_r2d4d)`).
